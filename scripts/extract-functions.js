...
const versionMatch = stateJsContent.match(
...
const stateObjectMatch = stateJsContent.match(
  /export const state = ({[\s\S]*?});/,
);
if (stateObjectMatch) {
...
const currenciesMatch = stateJsContent.match(
  /export const currencies = ({[\s\S]*?});/,
);
if (currenciesMatch) {
...
function extractTopLevelObject(source, varName) {
  ...
  while (i < source.length) {
    if (source[i] === "{") braceCount++;
      ...
      braceCount--;
      if (braceCount === 0) return source.substring(startIdx, i + 1);
    }
    i++;
  }
  return null;
...
const paymentMethodsMatch = stateJsContent.match(
  /export const PAYMENT_METHODS = (\[[\s\S]*?\]);/,
);
...
const expenseTypesMatch = stateJsContent.match(
  /export const EXPENSE_TYPES = (\[[\s\S]*?\]);/,
);
...
functions.forEach((fnName) => {
  ...
  if (!sourceCode) {
    console.warn(
      `⚠️  Module not found for function: ${fnName} (expected in ${sourceModule})`,
    );
    return;
  }
